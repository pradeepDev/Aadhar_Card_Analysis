...
object Highestaadhargenstates {
  def main(args:Array[String]){
    ...
    spark.sql("select  sum(AadharGenerated) as AadharGenerated,state from Aadhar group by state order by AadharGenerated desc limit 3 ").show()
