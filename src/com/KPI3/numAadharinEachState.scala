...
object numAadharinEachState {
   ...
   val df= spark.sql("select  sum(AadharGenerated) as AadharGenerated,state from Aadhar where AadharGenerated >0 group by state order by state ")
