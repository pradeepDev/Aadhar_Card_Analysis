...
object rejectedinUPandMaharatra {
  def main(args: Array[String]) {
   ...
   val df= spark.sql("select  sum(Rejected) as Rejected,state from Aadhar where Rejected >0 and State in('Uttar Pradesh','Maharashtra')group by state order by state ")
