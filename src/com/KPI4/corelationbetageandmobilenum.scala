...
object corelationbetageandmobilenum {
  def main(args: Array[String]) {
    ...
    val df = spark.sql("select  Cast (Age as Long) Age,sum(MobileNo) as TotalMobileNumberGiven,sum(AadharGenerated) as TotalaaDharGenerated,   (sum(MobileNo)/sum(AadharGenerated)*100)  from Aadhar  group by Age ")
