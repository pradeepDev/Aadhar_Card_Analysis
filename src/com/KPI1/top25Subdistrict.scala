...
object top25Subdistrict {
  ...
  val Data =  csvData.withColumnRenamed("_c0", "Date")
...
val df=spark.sql("select sum(AadharGenerated) as AadharGenerated  ,SubDistrict from Aadhar where  AadharGenerated >0 group by SubDistrict order by AadharGenerated desc limit 25 ")
